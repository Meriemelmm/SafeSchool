...
import { X, MapPin, Calendar, AlertTriangle, ShieldAlert, Paperclip, Users, Clock, AlertCircle, Loader2, PlayCircle, Headphones, FileText } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-gray-500 mb-2 flex items-center gap-1"><Users className="w-3 h-3" /> Signaleur</h4>
                    {signalement.isAnonymous ? (
                      <p className="font-medium text-sm italic text-gray-500 bg-gray-100 px-2 py-1 rounded inline-block">Anonyme</p>
            ...
            <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm min-h-[300px]">
              {loadingData ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {members.map(member => (
                    <div key={member._id} className="p-4 rounded-lg border border-gray-100 hover:border-indigo-100 hover:shadow-sm transition-all bg-gray-50/50">
                  ...
                  {preuves.map(preuve => {
                    ...
                    return (
                      <div key={preuve._id} className="group relative rounded-xl border border-gray-200 overflow-hidden bg-gray-50 hover:shadow-md transition-shadow">
                        ...
                        {/* Renderer selon le type de fichier */}
                        {preuve.fileType === 'image' && (
                          <div className="aspect-video relative overflow-hidden bg-gray-200">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={mediaUrl} alt="Preuve" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10">
                              <a href={mediaUrl} target="_blank" rel="noreferrer" className="px-4 py-2 bg-white/90 rounded-lg text-sm font-medium hover:bg-white text-gray-900 pointer-events-auto">Voir l'image</a>
                            </div>
                          </div>
                        )}

                        {preuve.fileType === 'video' && (
                          <div className="aspect-video relative overflow-hidden bg-black flex items-center justify-center">
                            ...
                            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-auto">
                              <a href={mediaUrl} target="_blank" rel="noreferrer" className="px-3 py-1 bg-white/20 hover:bg-white/40 backdrop-blur-sm text-white rounded text-xs font-semibold">Ouvrir</a>
                            </div>
                          </div>
                        )}

                        {preuve.fileType === 'audio' && (
                        ...
                        {(!preuve.fileType || preuve.fileType === 'document') && (
                          <div className="aspect-video flex flex-col items-center justify-center bg-gray-100 p-4 relative">
